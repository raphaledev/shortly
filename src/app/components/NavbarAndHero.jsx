    ...
    return (
        <section className="mx-8 mt-8 md:mx-0">
            ...
            <section className={`${visible ? 'flex' : 'hidden'} md:hidden my-8 w-full p-8 bg-slate-600 rounded-lg flex-col gap-4 text-center text-white font-semibold`}>
                ...
                <button className='bg-turquoise mx-8 py-1 rounded-2xl'>Sign Up</button>
            </section>
            <section className="m-8 mb-0 grid grid-cols-1 md:grid-cols-2 gap-16 md:w-5/6 md:mx-auto">
                <div className={`${!visible ? 'block' : 'hidden'} sm:max-w-full md:order-2 sm:block`}>
                    <Image className="w-full" src='/images/illustration-working.svg' alt="hero" width={500} height={500}>
                    </Image>
                </div>
                <div className="flex flex-col gap-4 text-center place-content-center md:gap-8">
                    <h1 className="text-4xl text-slate-dark font-bold">More than just shorter links</h1>
                    <p className="text-lavender-grey">Build your brand’s recognition and get detailed insights on how your links are performing.</p>
                    <button className="text-white bg-turquoise rounded-2xl py-1 px-8 mx-auto">Get Started</button>
                </div>
            </section>
        </section>
    )
